...
export default function CreatePage() {
  ...
  if (!session) {
    return (
      <EditableSiteShell>
        <main className="min-h-screen bg-[#030008] px-4 py-16 text-white sm:px-6 lg:px-8">
          <section className="editable-neon-panel mx-auto grid max-w-5xl gap-8 rounded-[2.8rem] p-7 md:grid-cols-[0.9fr_1.1fr] md:p-10">
            <div className="flex h-full min-h-72 items-center justify-center rounded-[2rem] border border-white/10 bg-[linear-gradient(135deg,rgba(0,240,200,0.18),rgba(117,35,255,0.22))] text-white">
              <Lock className="h-20 w-20 opacity-80" />
            </div>
            <div className="self-center">
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[var(--slot4-cyan)]">{pagesContent.create.locked.badge}</p>
              <h1 className="mt-5 text-5xl font-black leading-[0.92] tracking-[-0.08em] sm:text-7xl">{pagesContent.create.locked.title}</h1>
              <p className="mt-6 max-w-xl text-base font-semibold leading-8 text-white/68">{pagesContent.create.locked.description}</p>
              <div className="mt-8 flex flex-wrap gap-3">
                <Link href="/login" className="inline-flex items-center gap-2 rounded-[0.65rem] border border-[var(--slot4-cyan)] bg-[#1a1a1d] px-6 py-3 text-sm font-black">Sign in <ArrowRight className="h-4 w-4" /></Link>
                <Link href="/signup" className="inline-flex items-center gap-2 rounded-[0.65rem] border border-[var(--slot4-cyan)] bg-[var(--slot4-gradient)] px-6 py-3 text-sm font-black">Sign up</Link>
              </div>
            </div>
          </section>
        </main>
  ...
  return (
    <EditableSiteShell>
      <main className="min-h-screen bg-[#030008] text-white">
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
          <div className="editable-neon-panel grid gap-8 rounded-[2.8rem] p-6 lg:grid-cols-[0.85fr_1.15fr] lg:p-10">
            <aside>
            ...
            <form onSubmit={submit} className="rounded-[2.2rem] border border-white/10 bg-[#1a1a1d]/78 p-5 sm:p-7">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--slot4-cyan)]">Create {activeTask?.label || 'visual post'}</p>
                  <h2 className="mt-1 text-3xl font-black tracking-[-0.06em]">{pagesContent.create.formTitle}</h2>
                </div>
                <span className="rounded-full border border-white/10 bg-[#030008] px-4 py-2 text-xs font-black uppercase tracking-[0.16em] text-white/74">{session.name}</span>
              </div>
              ...
              {created ? (
                <div className="mt-5 rounded-2xl border border-[var(--slot4-cyan)] bg-[rgba(0,240,200,0.12)] p-4 text-white">
              ...
              <button type="submit" className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-2xl border border-[var(--slot4-cyan)] bg-[var(--slot4-gradient)] px-6 text-sm font-black uppercase tracking-[0.18em] text-white transition hover:-translate-y-0.5">
