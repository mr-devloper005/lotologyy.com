...
export function TaskDetailView({ task, post, related, comments = [] }: { task: TaskKey; post: SitePost; related: SitePost[]; comments?: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  const detailVars = { '--detail-bg': '#030008', '--detail-text': preset.colors.foreground, '--detail-surface': '#1a1a1d', '--detail-accent': '#00f0c8' } as CSSProperties
...
function BackLink({ task }: { task: TaskKey }) {
  ...
  return (
    <Link href={taskConfig?.route || '/'} className="inline-flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.06] px-4 py-2 text-sm font-black text-white">
...
function ArticleDetail({ post, related, comments }: { post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_350px] lg:px-8 lg:py-16">
      <article className="editable-neon-panel min-w-0 rounded-[2rem] p-5 sm:p-8 lg:p-12">
        <BackLink task="article" />
        <p className="mt-8 text-xs font-black uppercase tracking-[0.28em] text-[var(--detail-accent)]">{categoryOf(post, 'Article')}</p>
        <h1 className="mt-4 text-4xl font-black leading-tight tracking-normal sm:text-5xl lg:text-7xl">{post.title}</h1>
        {images[0] ? <img src={images[0]} alt="" className="mt-8 max-h-[620px] w-full rounded-[1.5rem] object-cover" /> : null}
  ...
  return (
    <section className="mx-auto max-w-[var(--editable-container)] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
        <article className="editable-neon-panel rounded-[2rem] p-6 sm:p-9">
...
function ImageDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="relative overflow-hidden px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      <div className="pointer-events-none absolute left-[-10rem] top-12 h-[520px] w-[520px] rounded-full bg-[radial-gradient(circle,rgba(0,240,200,0.22),rgba(116,39,255,0.16)_50%,transparent_74%)] blur-2xl" />
      <div className="pointer-events-none absolute right-[-12rem] top-64 h-[560px] w-[560px] rounded-full bg-[radial-gradient(circle,rgba(41,149,255,0.22),rgba(226,0,255,0.12)_55%,transparent_76%)] blur-2xl" />

      <div className="relative mx-auto max-w-[1280px]">
        <BackLink task="image" />
        <div className="mt-8 grid gap-6 lg:grid-cols-[0.9fr_1.1fr] lg:items-stretch">
          <article className="editable-neon-panel flex min-h-[520px] flex-col justify-between rounded-[2rem] p-7 sm:p-10">
            <div>
          ...
          <figure className="group relative min-h-[520px] overflow-hidden rounded-[2rem] border border-white/10 bg-[#1a1a1d] shadow-[0_30px_110px_rgba(0,0,0,0.34)]">
            <img src={feature} alt="" className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105" />
            <div className="absolute inset-0 bg-[linear-gradient(180deg,transparent_45%,rgba(3,0,8,0.78))]" />
            <figcaption className="absolute bottom-0 left-0 right-0 p-6 text-sm font-black text-white/78">Featured visual from this image post.</figcaption>
          </figure>
        </div>

        <div className="mt-8 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {gallery.slice(1).map((image, index) => (
            <figure key={`${image}-${index}`} className={`group overflow-hidden rounded-[1.65rem] border border-white/10 bg-[#1a1a1d] shadow-[0_22px_70px_rgba(0,0,0,0.28)] ${index % 4 === 0 ? 'md:col-span-2' : ''}`}>
              <img src={image} alt="" className={`w-full object-cover transition duration-700 group-hover:scale-105 ${index % 4 === 0 ? 'aspect-[16/8]' : 'aspect-[4/3]'}`} />
            </figure>
          ))}
        </div>
...
function BookmarkDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="editable-neon-panel rounded-[2rem] p-7 sm:p-10">
        <BackLink task="sbm" />
    ...
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="editable-neon-panel rounded-[2rem] p-6 sm:p-9">
        ...
        <div className="mt-8 grid gap-6 sm:grid-cols-[120px_1fr]">
          ...
          <div>
            <p className="text-xs font-black uppercase tracking-[0.28em] text-[var(--detail-accent)]">Visual file</p>
            <h1 className="mt-3 text-4xl font-black leading-tight tracking-normal sm:text-6xl">{post.title}</h1>
          </div>
        </div>
        <BodyContent post={post} />
...
function ProfileDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[var(--editable-container)] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[420px_minmax(0,1fr)] lg:px-8 lg:py-16">
      <aside className="editable-neon-panel rounded-[2rem] p-8 text-center lg:sticky lg:top-24 lg:self-start">
        ...
        <h1 className="mt-6 text-4xl font-black leading-tight tracking-normal">{post.title}</h1>
...
function MapBox({ src, label }: { src: string; label: string }) {
  return (
    <div className="overflow-hidden rounded-[1.5rem] border border-white/10 bg-[#1a1a1d] shadow-sm">
...
function ContactAction({ website, phone, email }: { website?: string; phone?: string; email?: string }) {
  ...
  return (
    <div className="mt-5 rounded-[1.5rem] border border-white/10 bg-[#1a1a1d] p-5 shadow-sm">
...
function RelatedPanel({ task, post: _post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-[1.65rem] border border-white/10 bg-white/[0.055] p-6 backdrop-blur">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--detail-accent)]">About this post</p>
          <div className="mt-4 grid gap-3 text-sm font-bold text-white/72 sm:grid-cols-2">
      ...
      {related.length ? (
        <div className="rounded-[1.65rem] border border-white/10 bg-white/[0.055] p-6 backdrop-blur">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-2xl font-black tracking-normal">More like this</h2>
            <Link href={taskConfig?.route || '/'} className="text-xs font-black uppercase tracking-[0.16em] text-white/55 transition hover:text-[var(--detail-accent)]">View all</Link>
          </div>
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-4 rounded-2xl border border-white/10 bg-[#1a1a1d] p-3 transition hover:-translate-y-0.5 hover:border-[var(--slot4-cyan)]">
      {image && task !== 'sbm' ? <img src={image} alt="" className="h-24 w-24 shrink-0 rounded-xl object-cover" /> : <div className="flex h-24 w-24 shrink-0 items-center justify-center rounded-xl bg-[var(--detail-bg)]"><FileText className="h-6 w-6 opacity-45" /></div>}
      <div className="min-w-0">
        <h3 className="line-clamp-3 text-sm font-black leading-tight tracking-normal">{post.title}</h3>
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-10 rounded-[1.5rem] border border-white/10 bg-white/[0.055] p-5">
      <div className="flex items-center gap-2 text-lg font-black"><MessageCircle className="h-5 w-5" /> Comments</div>
      <div className="mt-5 grid gap-3">
        {comments.slice(0, 5).map((comment) => (
          <div key={comment.id} className="rounded-2xl border border-white/10 bg-[#1a1a1d] p-4">
